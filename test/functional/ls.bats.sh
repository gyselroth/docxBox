...
@test "\"docxbox ls filename.docx\" outputs headlines of files and directories attributes" {
  ...
  #run grep -c 'Date' $BATS_TEST_DIRNAME/docxbox ls files/sample_100kB.docx
  #[[ "$output" = 1 ]]

  #run grep -c 'Time' $BATS_TEST_DIRNAME/docxbox ls files/sample_100kB.docx
  #[[ "$output" = 1 ]]

  #run grep -c 'Name' $BATS_TEST_DIRNAME/docxbox ls files/sample_100kB.docx
  #[[ "$output" = 1 ]]
}

@test "\"docxbox ls filename.docx\" outputs contained files" {
  run grep -c '_rels/.rels' $BATS_TEST_DIRNAME/docxbox ls files/sample_100kB.docx
