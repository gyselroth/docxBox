...
@test "Output of \"docxbox ls filename.docx\" contains files' and directories' attributes" {
  ...
  $BATS_TEST_DIRNAME/docxbox ls test/files/sample_100kB.docx | grep -c "Time"
...
@test "Output of \"docxbox ls filename.docx\" are contained files" {
  ...
  $BATS_TEST_DIRNAME/docxbox ls test/files/sample_100kB.docx | grep -c "word/_rels/document.xml.rels"
  $BATS_TEST_DIRNAME/docxbox ls test/files/sample_100kB.docx | grep -c "word/charts/chart1.xml"
  $BATS_TEST_DIRNAME/docxbox ls test/files/sample_100kB.docx | grep -c "word/document.xml"
  $BATS_TEST_DIRNAME/docxbox ls test/files/sample_100kB.docx | grep -c "word/fontTable.xml"
  $BATS_TEST_DIRNAME/docxbox ls test/files/sample_100kB.docx | grep -c "word/media/image1.jpeg"
  $BATS_TEST_DIRNAME/docxbox ls test/files/sample_100kB.docx | grep -c "word/numbering.xml"
  $BATS_TEST_DIRNAME/docxbox ls test/files/sample_100kB.docx | grep -c "word/settings.xml"
  $BATS_TEST_DIRNAME/docxbox ls test/files/sample_100kB.docx | grep -c "word/styles.xml"
}

@test "Output of \"docxbox ls filename.docx\" contains amount of contained files" {
  $BATS_TEST_DIRNAME/docxbox ls test/files/sample_100kB.docx | grep -c '11 files'
}
