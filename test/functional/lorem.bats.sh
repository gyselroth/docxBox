...
@test "With \"docxbox lorem filename.docx newFilename.docx\" text gets replaced by dummy text and is saved to new file" {
  run "$BATS_TEST_DIRNAME"/docxbox lorem test/functional/tmp/cp_table_unordered_list_images.docx test/functional/tmp/lorem.docx
  [ "$status" -eq 0 ]

  "$BATS_TEST_DIRNAME"/docxbox txt test/functional/tmp/lorem.docx | grep -vc "Culpa ad eiusmod"
